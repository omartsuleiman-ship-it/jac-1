...
export const DTC_DATABASE: Record<string, DTCRecord> = {
  ...
  P0218: {
    code: 'P0218',
    descEn: 'Transmission Fluid Over Temperature',
    descAr: 'ارتفاع خطير في حرارة زيت ناقل الحركة (الفتيس)',
    urgency: 'STOP',
    module: 'Transmission Control Module (TCM)',
    adviceEn: [
      'Pull over safely immediately and leave the engine idling in Park to let the transmission fluid cool down.',
      'Check underneath the vehicle for any leaking red/brown transmission fluid.',
      'Do not continue driving until the temperature drops; towing is recommended if it spikes again.',
      'Have the transmission cooler and fluid condition inspected at a workshop.',
      'Clear the code only after replacing burnt fluid or fixing the cooling circuit.',
    ],
    adviceAr: [
      'قف على جنب بأمان فوراً وسيب الموتور شغال على وضع الـ P عشان زيت الفتيس يبرد.',
      'بص تحت العربية واتأكد إن مفيش تسريب لزيت الفتيس (لونه أحمر أو بني).',
      'ماتكملش سواقة لحد ما الحرارة تنزل؛ ويُفضل تسحب العربية لو الحرارة رفعت تاني.',
      'افحص دورة تبريد الفتيس وحالة الزيت في ورشة متخصصة.',
      'امسح الكود فقط بعد تغيير الزيت المحروق أو إصلاح دورة التبريد.',
    ],
  },
  C0035: {
    code: 'C0035',
    descEn: 'Left Front Wheel Speed Sensor Circuit',
    descAr: 'عطل في دائرة حساس سرعة العجلة الأمامية اليسرى (ABS)',
    urgency: 'CAUTION',
    module: 'Anti-lock Braking System (ABS)',
    adviceEn: [
      'Drive with caution; standard brakes will work, but ABS and traction control may be disabled.',
      'Inspect the wiring harness for the left front wheel speed sensor for cuts or rubbing.',
      'Check the sensor connector for water intrusion or corrosion.',
      'Clean the magnetic sensor tip and the tone ring on the wheel hub from metal dust/debris.',
      'Replace the sensor if the circuit remains open after cleaning and checking wires.',
    ],
    adviceAr: [
      'سوق بحذر؛ الفرامل العادية هتشتغل، لكن نظام الـ ABS ومانع الانزلاق ممكن يكونوا فاصلين.',
      'افحص ضفيرة أسلاك حساس السرعة للعجلة الأمامية الشمال من أي قطع أو احتكاك.',
      'افحص فيشة الحساس من أي مياه أو صدأ.',
      'نضّف الحساس المغناطيسي والترس اللي على صرة العجلة من أي برادة حديد أو تراب.',
      'غيّر الحساس لو الدائرة فضلت مفتوحة بعد التنظيف وفحص الأسلاك.',
    ],
  },
  B1000: {
    code: 'B1000',
    descEn: 'Airbag ECU Malfunction',
    descAr: 'عطل داخلي في وحدة تحكم الوسائد الهوائية (الإيرباج)',
    urgency: 'STOP',
    module: 'Supplemental Restraint System (SRS)',
    adviceEn: [
      'Drive strictly to a repair shop; the airbag system is disabled and will not deploy in a crash.',
      'Do NOT attempt to probe or measure airbag wiring with a multimeter, as it can trigger deployment.',
      'Check the main fuses related to the SRS / Airbag system.',
      'If the vehicle was in a recent collision, the module may need to be reset or replaced.',
      'Have a professional scan and reprogram the Airbag ECU.',
    ],
    adviceAr: [
      'توجه فوراً لمركز صيانة؛ نظام الإيرباج فاصل ومش هيفتح في حالة الحوادث لا قدر الله.',
      'إياك تقيس أسلاك الإيرباج بالأفوميتر لأن ده ممكن يخليها تضرب في وشك.',
      'افحص الفيوزات العمومية الخاصة بنظام الـ SRS.',
      'لو العربية عملت حادثة قريب، الوحدة دي محتاجة إعادة برمجة أو تغيير.',
      'اطلب من فني متخصص فحص وبرمجة وحدة الإيرباج.',
    ],
  },
  C2120: {
    code: 'C2120',
    descEn: 'TPMS Sensor Signal Not Received',
    descAr: 'فقدان الاتصال مع حساس ضغط الإطارات',
    urgency: 'CAUTION',
    module: 'Tire Pressure Monitoring System (TPMS)',
    adviceEn: [
      'Stop safely and manually check all 4 tires with a pressure gauge to ensure none are flat.',
      'If pressures are fine, continue driving; the sensor battery may be dead.',
      'Identify which wheel sensor is not reporting data via the dashboard.',
      'Replace the TPMS sensor inside the affected wheel.',
      'Reprogram the new sensor to the vehicle ECU.',
    ],
    adviceAr: [
      'قف بأمان وقيس ضغط الـ 4 فرد كاوتش يدوياً عشان تتأكد إن مفيش عجلة نايمة.',
      'لو الضغط سليم، كمل سواقة عادي؛ غالباً بطارية الحساس الداخلي خلصت.',
      'حدد أي عجلة اللي فاصلة من خلال شاشة العدادات.',
      'غيّر حساس الـ TPMS اللي راكب جوه العجلة الفاصلة.',
      'برمج الحساس الجديد على كمبيوتر العربية.',
    ],
  },
};